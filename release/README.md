...
End-user archive contains Python runtime, backend dependencies, prebuilt frontend, anicli-api, ffmpeg, ready `.env`, `data/` and `library/`. Python/Node/Git are not required to run it. On macOS, the builder recursively bundles FFmpeg's non-system dylibs under `tools/lib/`, rewrites Mach-O load paths, ad-hoc signs the relocated binaries, rejects remaining Homebrew paths, and includes `scripts/grant-macos-permissions.sh` for repairing permissions lost during archive extraction.
